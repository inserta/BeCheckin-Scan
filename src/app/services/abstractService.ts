...
export class AbstractWS {
  ...
  protected makeGetRequest(path: string, paramsRequest: any, token?: string): Promise<any> {
    ...
    return this.getHeaders(token).then(headers => {
      return new Promise((resolve, reject) => {
        this.http.get(path, { headers: headers, params: paramsRequest }).toPromise().then(response => {
            resolve(response);
          }).catch(err => {
            if (err.status === 200) {
              resolve(null);
  ...
  protected makePostRequestWithoutHeaders(path: string, data: any): Promise<any> {
